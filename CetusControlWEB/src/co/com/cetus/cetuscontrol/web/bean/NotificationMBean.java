...
/**
 * The Class StatusMBean. Clase para el manejo de eventos de la gestión de
 * estados cetus controller
 * 
...
public class NotificationMBean extends GeneralManagedBean {
	...
	private List<NotificationDTO> listRegister = null;
	...
	public void initElement() {
		...
		if (userPortalDTO != null) {
			listRegister(userPortalDTO.getPerson().getClient().getClientCetus()
					.getId());
			...
			this.emailsPickList = new DualListModel<String>(
					emailsPickListSource, emailsPickListTarget);

		} else {
			try {
				getResponse().sendRedirect(
						getRequest().getContextPath()
								+ ConstantWEB.URL_PAGE_USER_NOVALID);
			} catch (Exception e) {
	...
	private void listRegister(int idClientCetus) {
		...
		try {
			response = generalDelegate.findNotifications(idClientCetus);
			if (UtilCommon.validateResponseSuccess(response)) {
				this.listRegister = (List<NotificationDTO>) response
						.getObjectResponse();
			} else {
				if (UtilCommon.validateResponseNoResult(response)) {
					this.listRegister = new ArrayList<NotificationDTO>();
				}
			}
		} catch (Exception e) {
	...
	public void validateSelectedRecord(ActionEvent event) {
		try {
			if (selectedObject != null) {
				...
				String[] emailsTemp = selectedObject.getMails() != null ? selectedObject
						.getMails().split(";") : null;
				...
				this.emailsPickListSource = listEmailsAdd != null ? listEmailsAdd
		...
		try {
			if (addObject != null) {
				if (validateMessages()) {
					...
					List<String> emails = this.emailsPickList.getTarget(); // Registros
																			// a
																			// almacenar
					String emailsAdd = "";
					...
					addObject.setMails(emailsAdd.substring(0,
							emailsAdd.length() - 1));
					NotificationTableDTO ntDTO = new NotificationTableDTO();
					ntDTO.setId(notificationTableCbox);
					addObject.setNotificationTable(ntDTO);
					this.showConfirmAdd = true;

				} else {
					addMessageError("notificationAddMsg",
							ConstantWEB.MESSAGE_ERROR_CHEK_NOTACTIONS, null);
				}
	...
	boolean validateMessages() {
		boolean flag = true;
		if (addObject.isCreateAction()
				&& addObject.getCreateActionMessage().isEmpty())
			flag = false;
		if (!addObject.isCreateAction()
				&& !addObject.getCreateActionMessage().isEmpty())
			flag = false;
		if (addObject.isUpdateAction()
				&& addObject.getUpdateActionMessage().isEmpty())
			flag = false;
		if (!addObject.isUpdateAction()
				&& !addObject.getUpdateActionMessage().isEmpty())
			flag = false;
		if (addObject.isDeleteAction()
				&& addObject.getDeleteActionMessage().isEmpty())
			flag = false;
		if (!addObject.isDeleteAction()
				&& !addObject.getDeleteActionMessage().isEmpty())
			flag = false;
		if (!addObject.isCreateAction() && !addObject.isUpdateAction()
				&& !addObject.isDeleteAction()
				&& addObject.getCreateActionMessage().isEmpty()
				&& addObject.getUpdateActionMessage().isEmpty()
				&& addObject.getDeleteActionMessage().isEmpty())
			flag = false;
		...
		try {
			if (selectedObject != null) {
				...
				if (notificationDTO != null) {
					...
					selectedObject.setClientCetus(notificationDTO
							.getClientCetus());
					selectedObject.setCreateDate(notificationDTO
							.getCreateDate());
					selectedObject.setCreateUser(notificationDTO
							.getCreateUser());
					selectedObject.setIdNotTable(notificationDTO
							.getIdNotTable());
					selectedObject.setModificationUser(getUserInSession());

					if (notificationDTO.isNoTableSystem()) {
						selectedObject.setNoTableSystem(notificationDTO
								.isNoTableSystem());
						selectedObject.setCreateActionMessage(notificationDTO
								.getCreateActionMessage());
						selectedObject.setCreateAction(notificationDTO
								.isCreateAction());
						selectedObject.setUpdateActionMessage(notificationDTO
								.getUpdateActionMessage());
						selectedObject.setUpdateAction(notificationDTO
								.isUpdateAction());
						selectedObject.setDeleteActionMessage(notificationDTO
								.getDeleteActionMessage());
						selectedObject.setDeleteAction(notificationDTO
								.isDeleteAction());
						selectedObject.setOwnerNotify(notificationDTO
								.isOwnerNotify());
					}

					List<String> emails = this.emailsPickList.getTarget(); // Emails
																			// actualizar
					if (emails != null && emails.size() > 0) {
						...
						selectedObject.setMails(emailsAdd.substring(0,
								emailsAdd.length() - 1));
					}
				}
	...
	private void listRegisterNoTable() {
		ResponseDTO response = null;
		try {
			response = generalDelegate
					.findNotificationsTableNoSystem(getUserDTO().getPerson()
							.getClient().getClientCetus().getId());
			if (UtilCommon.validateResponseSuccess(response)) {
				// Respuesta del servicio
				this.listRegisterNoTable = (List<NotificationTableDTO>) response
						.getObjectResponse();
			} else {
	...
	private List<String> loadListEmails() {
		...
		try {
			response = generalDelegate.findEmailsFromPerson(getUserDTO()
					.getPerson().getClient().getClientCetus().getId());
			if (UtilCommon.validateResponseSuccess(response)) {
				// Respuesta del servicio
	...
	public String remove() {
		...
		try {
			...
			if (selectedObject != null) {
				if (!selectedObject.isNoTableSystem()) {
					ConstantWEB.WEB_LOG
							.debug("########## REGISTRO A ELIMINAR  "
									+ selectedObject.getDescription());
					response = generalDelegate.remove(selectedObject);
					if (UtilCommon.validateResponseSuccess(response)) {
						ConstantWEB.WEB_LOG
								.debug("########## REGISTRO ELIMINADO  "
										+ selectedObject.getDescription());
						this.initElement();
						addMessageInfo(null, ConstantWEB.MESSAGE_SUCCES,
								ConstantWEB.MESSAGE_SUCCES_DELETE);
					} else {
						ConstantWEB.WEB_LOG
								.debug("########## ERROR ELIMINADO EL REGISTRO  "
										+ selectedObject.getDescription());
						addMessageError(null, ConstantWEB.MESSAGE_ERROR,
								ConstantWEB.MESSAGE_ERROR_DELETE_DETAIL);
					}
				} else {
					ConstantWEB.WEB_LOG
							.debug("########## NO SE PUEDE ELIMINAR NOTIFICACION  "
									+ selectedObject.getDescription());
					addMessageError(null, ConstantWEB.MESSAGE_ERROR,
							ConstantWEB.MESSAGE_ERROR_DELETE_NOTACTIONS);
				}
			}
		} catch (Exception e) {
			ConstantWEB.WEB_LOG.error(e.getMessage(), e);
			addMessageError(null, ConstantWEB.MESSAGE_ERROR,
					ConstantWEB.MSG_DETAIL_ERROR);
		}
		return null;
	}
	...
	public String update() {
		...
		try {
			...
			if (selectedObject != null) {
				...
				ConstantWEB.WEB_LOG
						.debug("######### ACTUALIZAR EL SIGUIENTE REGISTRO "
								+ selectedObject.getDescription());
				response = generalDelegate.edit(selectedObject);
				if (UtilCommon.validateResponseSuccess(response)) {
					ConstantWEB.WEB_LOG
							.debug("######### REGISTRO ACTUALIZADO CORRECTAMENTE "
									+ selectedObject.getDescription());
					this.initElement();
					lSuccessfull = true;
					addMessageInfo(null, ConstantWEB.MESSAGE_SUCCES,
							ConstantWEB.MESSAGE_SUCCES_UPDATE);
				} else {
					// Obetener codigo de error de oracle
					String code = "0";
					if (!response.getMessage().isEmpty()
							&& response.getMessage().contains("ORA")) {
						code = response.getMessage().substring(
								response.getMessage().lastIndexOf("ORA"),
								response.getMessage().lastIndexOf(":"));
						addMessageError(null, ConstantWEB.MESSAGE_ERROR,
								"Codigo de Error: " + code);
					} else {
						addMessageError(null, ConstantWEB.MESSAGE_ERROR,
								ConstantWEB.MESSAGE_ERROR_UPDATE);
					}
					ConstantWEB.WEB_LOG
							.debug("######### ERROR ACTUALIZANDO EL REGISTRO"
									+ selectedObject.getDescription());
				}
			}
		} catch (Exception e) {
	...
	public String add() {
		...
		try {
			...
			if (addObject != null) {
				if (getUserDTO() != null) {
					addObject.setClientCetus(getUserDTO().getPerson()
							.getClient().getClientCetus());
				}
				context = RequestContext.getCurrentInstance();
				...
				if (UtilCommon.validateResponseSuccess(response)) {
					ConstantWEB.WEB_LOG
							.debug("..::Notificacion creada exitosamente::.."
									+ addObject.getDescription());
					this.initElement();
					lSuccessfull = true;
					addMessageInfo(null, ConstantWEB.MESSAGE_SUCCES,
							ConstantWEB.MSG_DETAIL_SUCCESS);
				} else {
					// Obetener codigo de error de oracle
					String code = "0";
					if (!response.getMessage().isEmpty()
					...
					addMessageError(null, ConstantWEB.MESSAGE_ERROR_CREATE,
							code + ":" + ConstantWEB.MSG_DETAIL_ERROR);
				}
			}
		} catch (Exception e) {
			ConstantWEB.WEB_LOG.error(e.getMessage(), e);
	...
	public List<NotificationDTO> getListRegister() {
		return listRegister;
	}

	public void setListRegister(List<NotificationDTO> listRegister) {
	...
	public int getNotificationTableCbox() {
	...
	public void setListRegisterNoTable(
			List<NotificationTableDTO> listRegisterNoTable) {
